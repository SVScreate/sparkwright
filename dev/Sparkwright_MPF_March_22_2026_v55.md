...
140. **Student Dashboard** *(formerly My Progress — redesign spec from Spark, Session Z)* — Three sections: **Section 1 — Fact Constellation** (hero visual, 12×12 grid, all facts always visible, dim = unpracticed, color = tier, glow intensity = how established, filterable by operation, print pill, student name + date, Pip star badge at mastery). Aesthetic: warm forge/alchemy energy — NOT cold space. **Section 2 — Facts to Watch** (challenge facts, near-mastery, regressed — replaces current tile pile). **Section 3 — Assessment Records** (assessment history by date, "Run Assessment" button here). My Progress tier improvements and item 143 (print pill) both hold until this redesign is built. Do not finalize title screen (item 100) until dashboard access is confirmed.

157. **Dashboard animation pass** *(Spark Session Z — log, don't build yet)* — "What's new" spark-pop when dashboard opens (newly mastered facts since last visit animate briefly, replay button). Change-over-time slider animating constellation between two assessments (defer until comparison view exists).

158. **Threshold visibility on dashboard** *(Spark Session Z — add to items 130/139)* — display current fluency threshold on My Progress/Dashboard. Small, contextual — e.g. "Fluency graded at 3-second threshold." Not a button. Just tells you what you're looking at.

159. **Threshold-change data integrity** *(Spark Session Z — design before building)* — when the fluency threshold is changed, what happens to existing practice data tier colors on the dashboard? Assessment records are safe (stamped with threshold). Practice data is the open question. Do not build the threshold-change mechanic until this is resolved. Connects to items 130, 131, 106.
